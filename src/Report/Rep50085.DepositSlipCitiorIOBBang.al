report 50085 "Deposit Slip Citi orIOB-Bang"
{
    DefaultLayout = RDLC;
    RDLCLayout = './res/DepositSlipCitiorIOBBang.rdlc';

    dataset
    {
        dataitem("Collection Detail"; "Collection Detail")
        {
            DataItemTableView = WHERE("Deposit Bank" = FILTER('BANK-006' | 'BANK-007' | 'BANK-008' | 'BANK-009'), Location = FILTER('BANGALORE'), "On Submit Receipt No." = FILTER(<> ''));
