report 50064 "DepositSlip HDFC=HDFC-Ahme"
{
    DefaultLayout = RDLC;
    RDLCLayout = './res/DepositSlipHDFCHDFCAhme.rdlc';

    dataset
    {
        dataitem("Collection Detail"; "Collection Detail")
        {
            DataItemTableView = WHERE("Client Bank Name" = FILTER('*HDFC*'), Location = FILTER('AHMEDABAD'), "On Submit Receipt No." = FILTER(<> ''));
