report 50034 "Deposit Slip Citi or IOB"
{
    DefaultLayout = RDLC;
    RDLCLayout = './res/DepositSlipCitiorIOB.rdlc';

    dataset
    {
        dataitem("Collection Detail"; "Collection Detail")
        {
            DataItemTableView = WHERE("Deposit Bank" = FILTER('BANK-006' | 'BANK-007' | 'BANK-008' | 'BANK-009'), "On Submit Receipt No." = FILTER(<> ''));
