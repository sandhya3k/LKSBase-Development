report 50003 INVOICE_IBL_FINAL
{
    ...
    ApplicationArea = All;
    DefaultLayout = RDLC;
    RDLCLayout = './res/INVOICEIBLFINAL.rdlc';

    Caption = 'INVOICE_IBL_FINAL';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(DataItem1000000018; Direct_remittance_Details__000Lbl)
            {
            }
            column(CurrencyCode_SalesInvoiceHeader; "Sales Invoice Header"."Currency Code")
            {
            }
            column(Add1; "Sales Invoice Header"."Sell-to Address")
            {
            }
            column(Add2; "Sales Invoice Header"."Sell-to Address 2")
            {
            }
            column(City; "Sales Invoice Header"."Sell-to City" + ' ' + "Sales Invoice Header"."Bill-to Post Code")
            {
            }
            column(Contact; "Sales Invoice Header"."Sell-to Contact")
            {
            }
            column(Event_Date; DimValuName)
            {
            }
            column("Event"; 'EVENT' + '-    ' + DimValuName1)
            {
            }
            column(Venue; 'VENUE' + '-     ' + DimValuName2)
            {
            }
            column(Category_Category; Category)
            {
            }
            ...
            column(Account_No00322320002210; Account_No__00322320002210)
            {
            }
            column(IBL_QUOTE; IBLQUOTE)
            {
            }
            column(SwachVar_Amt; Round(SwachVar, 0.01))
            {
            }
            column(SBCESS; SBCess)
            {
            }
            column(KKC_Cess; KKCess)
            {
            }
            column(KKC_Amt; Round(KKCvar, 0.01))
            {
            }
            column(ServiceAmtVar_Amt; Round(ServiceAmtVar, 0.01))
            {
            }
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(Sales_Invoice_Header___Post_Office_______Sales_Invoice_Header___Ship_to_City______PinCode1; "Sales Invoice Header"."Post Office" + ' ' + "Sales Invoice Header"."Ship-to City")
                    {
                    }
                    column(STNo; STNo)
                    {
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        begin
                            ...
                            */
                            ...
                            CheckReport.FormatNoText(NumberText, Round(LineTotAmt, 1), "Sales Invoice Header"."Currency Code");
                            ...
                            //TBD
                            /*
                            if "Sales Invoice Line"."Service Tax Group" <> '' then begin
                                ServTaxSetup.Reset;
                                ServTaxSetup.SetRange(Code, "Sales Invoice Line"."Service Tax Group");
                                ServTaxSetup.SetFilter("From Date", '<=%1', "Sales Invoice Header"."Posting Date");
                                if ServTaxSetup.FindLast then
                                    ServiceTaxP := ServTaxSetup."Service Tax %";
                            end;
                            */
                            //TBD //ServiceTax := ServiceTax + "Sales Invoice Line"."Service Tax Amount";

                            SBCess := '';
                            if "Sales Invoice Line"."Posting Date" > 20151115D then
                                SBCess := 'SB Cess @0.50%';
                            //    MESSAGE('SBCess') ;

                            if ServiceTaxP = 14.5 then begin
                                ServiceTaxP2 := 14;
                                SwachVar := ServiceTax * 0.0344827;
                                //    MESSAGE('%1',SwachVar);
                                ServiceAmtVar := ServiceTax - SwachVar;
                                //    MESSAGE('%1',ServiceAmtVar);
                            end
                            else begin
                                ServiceTaxP2 := ServiceTaxP;
                                SwachVar := 0;
                                ServiceAmtVar := ServiceTax - SwachVar;
                            end;
                            ...
                            KKCvar := 0;
                            //SwachVar:=0;
                            if ServiceTaxP = 15 then begin
                                ServiceTaxP2 := 14;
                                SwachVar := ServiceTax * 0.033333;
                                if "Sales Invoice Line"."Posting Date" >= 20160601D then begin
                                    KKCess := 'KK Cess @0.50%';
                                    KKCvar := ServiceTax * 0.033333;
                                    //ServiceAmtVarkkc := ServiceTax -KKCvar;
                                end;
                                SBKKCessAmt := SwachVar + KKCvar;
                                //MESSAGE('%1',SBKKCessAmt);
                                ServiceAmtVar := ServiceTax - SBKKCessAmt;
            ...
            begin
                ...
                DimSetEntry.SetRange(DimSetEntry."Dimension Code", 'EVENT DATE');
                if DimSetEntry.FindSet then
                    DimValu := DimSetEntry."Dimension Value Code";
                DimValues.Reset;
                DimValues.SetFilter(DimValues."Dimension Code", 'EVENT DATE');
                DimValues.SetRange(DimValues."Dimension Value ID", DimSetEntry."Dimension Value ID");
                DimValues.SetFilter(DimValues.Code, DimValuName);
                if DimValues.FindSet then
                    DimValuName := DimValues.Name;

                DimValu1 := '';
                DimValuName1 := '';
                DimSetEntry.Reset;
                DimSetEntry.SetRange(DimSetEntry."Dimension Set ID", "Dimension Set ID");
                DimSetEntry.SetRange(DimSetEntry."Dimension Code", 'EVENT');
                if DimSetEntry.FindSet then
                    DimValu1 := DimSetEntry."Dimension Value Code";

                DimValues.Reset;
                DimValues.SetFilter(DimValues."Dimension Code", 'EVENT');
                DimValues.SetFilter(DimValues.Code, DimValu1);
                if DimValues.FindSet then
                    DimValuName1 := DimValues.Name;

                DimValu2 := '';
                DimValuName2 := '';
                DimSetEntry.Reset;
                DimSetEntry.SetRange(DimSetEntry."Dimension Set ID", "Dimension Set ID");
                DimSetEntry.SetRange(DimSetEntry."Dimension Code", 'EVENT VENUE');
                if DimSetEntry.FindSet then
                    DimValu2 := DimSetEntry."Dimension Value Code";
                DimValues.Reset;
                DimValues.SetFilter(DimValues."Dimension Code", 'EVENT VENUE');
                DimValues.SetRange(DimValues."Dimension Value ID", DimSetEntry."Dimension Value ID");
                DimValues.SetFilter(DimValues.Code, DimValu2);
                if DimValues.FindSet then
                    DimValuName2 := DimValues.Name;
                //TEAM::1426
                ...
                */
                //   Stax :=ServiceTaxEntry."Service Tax Amount";
                ...
                */
                ...
                ServiceTaxP := 0;
                SilRec.Reset;
                SilRec.SetRange("Document No.", "No.");
                if SilRec.FindSet then
                    repeat
                        ; //TBD //SwachVar += SilRec."Service Tax Amount";
                    until SilRec.Next = 0;
                SwachVar := ServiceTax * 0.0344827;
                //  MESSAGE('%1',SwachVar);
                ServiceAmtVar := ServiceTax - SwachVar;
    ...
    var
        ...
        Account_No__00322260000024: Label 'NOTE:Direct remittance Details - Account No. 00322260000024 with HDFC BANK C-5/32 SAFDARJUNG DEVELOPMENT AREA, NEW DELHI - 110016, INDIA, Account Type - Current, MICR - 1 1 0240004  IFSC Code HDFC0000032. The name of the account holder is V. SRIDHARAN';
        EventDate: Text[100];
        DimSetEntry: Record "Dimension Set Entry";
        DimValuName: Text[100];
        DimValu: Text[100];
        DimValuName1: Text[100];
        DimValuName2: Text[100];
        DimValu1: Text[100];
        DimValu2: Text[100];
        Account_No__00322320002210: Label 'NOTE:Direct Remittance Details:- Account No. 00322320002210 with HDFC BANK C-5/32 SAFDARJUNG DEVELOPMENT AREA, NEW DELHI - 110016, IFSC/RTGS: HDFC0000032. The name of the account holder is INSTITUTE OF BUSINESS LAWS';
        IBLQUOTE: Label 'KINDLY PAY IN FAVOUR OF ''INSTITUTE OF BUSINESS LAWS'' AND QUOTE THIS MEMO NO. WHILE MAKING PAYMENT';
        ServiceTaxP: Decimal;
        SBCess: Text[100];
        ServiceTaxP2: Decimal;
        SwachVar: Decimal;
        ServiceAmtVar: Decimal;
        ServiceTax: Decimal;
        SilRec: Record "Sales Invoice Line";
        KKCess: Text;
        KKCessDec: Decimal;
        KKCvar: Decimal;
        ServiceAmtVarkkc: Decimal;
        SBCImplmntDate: Decimal;
        KKCImplmntDate: Date;
        ServiceTaxP3: Decimal;
        SBKKCessAmt: Decimal;
