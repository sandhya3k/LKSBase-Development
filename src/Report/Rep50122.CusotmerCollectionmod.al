report 50122 "Cusotmer Collection_mod"
{
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = All;
    Caption = 'Cusotmer Collection_mod';
    DefaultLayout = RDLC;
    RDLCLayout = './res/CusotmerCollectionmod.rdlc';
    ...
    dataset
    {
        dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
        {
            ...
            column(NetCollINR; (-Amount_1) - Reim_Exp - (-ServiceTaxAmt))
            {
            }
            column(Invoice_Amount; Amt2)
            {
            }
            ...
            column(Group_Head; "Cust. Ledger Entry"."Group Heade")
            {
            }
            ...
            begin
                CurrReport.CreateTotals("Cust. Ledger Entry"."Amount (LCY)", "Original Amt. (LCY)", Amount_1, Amt2);
                SetFilter("Posting Date", '%1..%2', DateFilterNew, EndingDateNew);
                if SubGrpGlobalCheck then
                    SetFilter("Sub Group Owner 2", SubGrpGlobal)
                else
                    SetFilter("Group Heade", GroupHeadFilterNew);
            end;
        }
    }
    ...
    var
        ...
        SubGrpGlobal: Code[20];
        SubGrpGlobalCheck: Boolean;
    ...
    procedure Setfilters(DateFilter: Date; EndingDate: Date; OwnerFilter: Code[20]; SubGroupFilter: Code[20]; SubGroupFilterCheck: Boolean)
    begin
        DateFilterNew := DateFilter;
        EndingDateNew := EndingDate;
        GroupHeadFilterNew := OwnerFilter;
        SubGrpGlobal := SubGroupFilter;
        SubGrpGlobalCheck := SubGroupFilterCheck;
    end;
}
