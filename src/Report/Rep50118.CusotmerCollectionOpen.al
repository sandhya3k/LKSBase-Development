report 50118 "Cusotmer Collection Open"
{
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = All;
    Caption = 'Cusotmer Collection Open';
    DefaultLayout = RDLC;
    RDLCLayout = './res/CusotmerCollectionOpen.rdlc';

    dataset
    {
        dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
        {
            DataItemTableView = SORTING("Entry No.") ORDER(Ascending) WHERE("Document Type" = FILTER(Payment | " "), Reversed = FILTER(false), "Amount (LCY)" = FILTER(< 0), "Document No." = FILTER(<> '@JVWO*' & <> '@BC*'), "Applied Document (Invoice)" = FILTER(<> '@JV*' & <> '@BC*' & <> '@BP*'), "Applied Document" = FILTER(<> '@JVPI*' & <> '@BC*'), "Journal Batch Name" = FILTER(<> '@BOUNCE*'), Open = FILTER(true));
            ...
            begin
                ...
                */
                ...
                Narration := '';
                Narr.Reset;
                Narr.SetRange(Narr."Document No.", "Cust. Ledger Entry"."Document No.");
                if Narr.FindFirst then
                    repeat
                        Narration := Narration + Narr.Narration;
                    until
                    Narr.Next = 0;
