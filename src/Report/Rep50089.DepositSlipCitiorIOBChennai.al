report 50089 "Deposit Slip CitiorIOB-Chennai"
{
    DefaultLayout = RDLC;
    RDLCLayout = './res/DepositSlipCitiorIOBChennai.rdlc';

    dataset
    {
        dataitem("Collection Detail"; "Collection Detail")
        {
            DataItemTableView = WHERE("Deposit Bank" = FILTER('BANK-006' | 'BANK-007' | 'BANK-008' | 'BANK-009'), Location = FILTER('CHENNAI'), "On Submit Receipt No." = FILTER(<> ''));
