report 50073 "Cusotmer Collection"
{
    UsageCategory = ReportsAndAnalysis;
    ApplicationArea = All;
    DefaultLayout = RDLC;
    RDLCLayout = './res/CusotmerCollection.rdlc';
    ...
    dataset
    {
        dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
        {
            ...
            column(SerTaxAmt; -ServiceTaxAmt)
            {
            }
            column(NetColAmt; (-Amount1) - Reim_Exp - (-ServiceTaxAmt))
            {
            }
            column(GroupHead_Name; GroupHeadName)
            {
            }
            ...
            begin
                ...
                if GL_Entry.Find('-') then
                    //TBD //Location_Code := GL_Entry."Location Code";
                    Location_Code := '';//3k-UPG
                ...
                if D_V_CLE.FindFirst then begin
                    Amount1 := D_V_CLE."Amount (LCY)";
                    if (IncomeHead = 'Service Tax Payable') then
                ...
                if SalesInvHeader.FindSet then
                    ;//TBD //SalesInvHeader.CalcFields(SalesInvHeader."Amount to Customer");
                //TBD //Amt2 := SalesInvHeader."Amount to Customer";
                // MESSAGE('%1',Amt2);

                Rec_CLEM.Reset;
                Rec_CLEM.SetRange(Rec_CLEM."Entry No.", "Entry No.");
                Rec_CLEM.SetRange(Rec_CLEM."Document No.", "Document No.");
                Rec_CLEM.SetFilter(Rec_CLEM."Customer Posting Group", 'REFUND');
                Rec_CLEM.SetRange(Rec_CLEM.Open, false);
                if Rec_CLEM.FindFirst then
                    CurrReport.Skip;
                ...
                DimensionValue.SetRange(Code, "Group Heade");
                if DimensionValue.Find('-') then
                    GroupHeadName := DimensionValue.Name;
                ...
                if "Applied Document (Invoice)" <> '' then begin
                    ...
                    if "Applied Document" <> '' then begin
                        ...
                        Dim.Reset;
                        Dim.SetFilter(Dim."Dimension Code", 'CASE');
                        Dim.SetFilter(Dim.Code, "Cust. Ledger Entry"."Global Dimension 2 Code");
                        if Dim.FindFirst then begin
                            OWNER1 := Dim."Case-Owner1";
                            OWNER2 := Dim."Case-Owner2";
                        end;
                    end;

                if Dim.Get('SUBGRP-OWNER2', "Cust. Ledger Entry"."Sub Group Owner 2") then
                    SubGroupName := Dim.Name;
            end;

            trigger OnPreDataItem()
            begin
                CurrReport.CreateTotals("Cust. Ledger Entry"."Amount (LCY)", "Original Amt. (LCY)", Amount1, Amt2);
                //"Cust. Ledger Entry".SETRANGE("Document No.",'BRPI/HY/1718/08/0007');
            end;
        }
    }
    ...
    var
        ...
        Rec_CLEM: Record "Cust. Ledger Entry";
