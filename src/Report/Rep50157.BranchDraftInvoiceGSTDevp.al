report 50157 "Branch Draft Invoice GST(Devp)"
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './res/BranchDraftInvoiceGSTDevp.rdlc';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(CommercialBollean; CommercialBollean)
            {
            }
            column(HSNVar; HSNVar)
            {
            }
            column(CategoryVar; CategoryVar)
            {
            }
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem("Sales Line"; "Sales Line")
                    {
                        ...
                        column(GstBaseValue; GstBaseValue)
                        {
                        }
                        ...
                        begin
                            Clear(GstBaseValue);
                            Clear(IGSTPercentageVar);
                            Clear(CGSTPercentageVar);
                            Clear(SGSTPercentageVar);
                            ...
                            */
                            ...
                            DetailedGstLedgerBufferRec.Reset;
                            DetailedGstLedgerBufferRec.SetRange("Transaction Type", DetailedGstLedgerBufferRec."Transaction Type"::Sales);
                            DetailedGstLedgerBufferRec.SetRange("Document Type", DetailedGstLedgerBufferRec."Document Type"::Invoice);
                            DetailedGstLedgerBufferRec.SetRange("Document No.", "Sales Line"."Document No.");
                            DetailedGstLedgerBufferRec.SetRange("Line No.", "Sales Line"."Line No.");
                            DetailedGstLedgerBufferRec.SetRange("GST Component Code", 'IGST');
                            if DetailedGstLedgerBufferRec.FindFirst then begin
                                GstBaseValue := Abs(DetailedGstLedgerBufferRec."GST Base Amount");
                                IGSTAmountVar += Abs(DetailedGstLedgerBufferRec."GST Amount");
                                IGSTPercentageVar := DetailedGstLedgerBufferRec."GST %";
                            end;
                            ...
                            DetailedGstLedgerBufferRec.SetRange("Document No.", "Sales Line"."Document No.");
                            DetailedGstLedgerBufferRec.SetRange("Line No.", "Sales Line"."Line No.");
                            DetailedGstLedgerBufferRec.SetRange("GST Component Code", 'CGST');
                            if DetailedGstLedgerBufferRec.FindFirst then begin
                                GstBaseValue := Abs(DetailedGstLedgerBufferRec."GST Base Amount");
                                CGSTAmountVar += Abs(DetailedGstLedgerBufferRec."GST Amount");
                                CGSTPercentageVar := DetailedGstLedgerBufferRec."GST %";
                            end;
                            DetailedGstLedgerBufferRec.Reset;
                            DetailedGstLedgerBufferRec.SetRange("Transaction Type", DetailedGstLedgerBufferRec."Transaction Type"::Sales);
                            DetailedGstLedgerBufferRec.SetRange("Document Type", DetailedGstLedgerBufferRec."Document Type"::Invoice);
                            DetailedGstLedgerBufferRec.SetRange("Document No.", "Sales Line"."Document No.");
                            DetailedGstLedgerBufferRec.SetRange("Line No.", "Sales Line"."Line No.");
                            DetailedGstLedgerBufferRec.SetRange("GST Component Code", 'SGST');
                            if DetailedGstLedgerBufferRec.FindFirst then begin
                                GstBaseValue := Abs(DetailedGstLedgerBufferRec."GST Base Amount");
                                SGSTAmountVar += Abs(DetailedGstLedgerBufferRec."GST Amount");
                                SGSTPercentageVar := DetailedGstLedgerBufferRec."GST %";
                            end;

                            CheckReport.InitTextVariable;
                            CheckReport.FormatNoText(NumberText, Round(LineTotAmt + IGSTAmountVar + CGSTAmountVar + SGSTAmountVar, 1), "Sales Header"."Currency Code");
                    ...
                    dataitem("Integer"; "Integer")
                    {
                        ...
                        column(IGSTAmountVar; IGSTAmountVar)
                        {
                        }
                        column(IGSTPercentageVar; IGSTPercentageVar)
                        {
                        }
                        column(CGSTAmountVar; CGSTAmountVar)
                        {
                        }
                        column(SGSTAmountVar; SGSTAmountVar)
                        {
                        }
                        column(CGSTPercentageVar; CGSTPercentageVar)
                        {
                        }
                        column(SGSTPercentageVar; SGSTPercentageVar)
                        {
                        }
            ...
            begin
                ...
                */
                ...
                REc_State.Reset;
                REc_State.SetRange(Description, "Sales Header".ShipToStateName);
                if REc_State.FindFirst then
                    StateCodeVar := "Sales Header".ShipToStateName + ' - ' + REc_State."State Code (GST Reg. No.)";

                if CommercialBollean = true then begin
                    HSNVar := '9992';
                    CategoryVar := 'Commercial training and coaching services';
                end
                else begin
                    HSNVar := '9992';
                    CategoryVar := 'LEGAL SERVICES';
                end;
            end;
            ...
            begin
                ...
                CaptiontobeAdded := 'KINDLY PAY IN FAVOUR OF "' + UpperCase(CompanyInfo.Name) + '" AND QUOTE THIS INVOICE NO. WHILE MAKING PAYMENT';
            end;
        }
    }
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field("Commercial Training"; CommercialBollean)
                    {
                        ApplicationArea = All;
                    }
                }
    ...
    var
        ...
        KINDLY_PAY_IN_FAVOUR_OF__LAKSHMI_KUMARAN___SRIDHARAN__Lbl: Label 'KINDLY PAY IN FAVOUR OF ''LAKSHMI KUMARAN & SRIDHARAN'' AND QUOTE THIS INVOICE NO. WHILE MAKING PAYMENT';
        place1: Decimal;
        ...
        ShipLocation: Text[60];
        REc_State: Record State;
        State_Code: Code[10];
        StateDesc: Text;
        ShiptoPostCode: Code[500];
        StateCodeVar: Code[60];
        BankDet: Text[1000];
        CaptiontobeAdded: Text[200];
        DetailedGstLedgerBufferRec: Record "Detailed GST Entry Buffer";
        GstBaseValue: Decimal;
        IGSTAmountVar: Decimal;
        IGSTPercentageVar: Decimal;
        CGSTAmountVar: Decimal;
        CGSTPercentageVar: Decimal;
        SGSTAmountVar: Decimal;
        SGSTPercentageVar: Decimal;
        CommercialBollean: Boolean;
        HSNVar: Code[5];
        CategoryVar: Text[50];
