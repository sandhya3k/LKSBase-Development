report 50055 "DepositSlip HDFC=HDFC-Mumbai"
{
    DefaultLayout = RDLC;
    RDLCLayout = './res/DepositSlipHDFCHDFCMumbai.50055.rdlc';

    dataset
    {
        dataitem("Collection Detail"; "Collection Detail")
        {
            DataItemTableView = WHERE("Client Bank Name" = FILTER('*HDFC*'), Location = FILTER('MUMBAI'), "On Submit Receipt No." = FILTER(<> ''));
