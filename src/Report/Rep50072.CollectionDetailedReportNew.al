report 50072 "Collection Detailed Report New"
{
    DefaultLayout = RDLC;
    RDLCLayout = './res/CollectionDetailedReportNew.rdlc';

    dataset
    {
        dataitem("G/L Entry"; "G/L Entry")
        {
            DataItemTableView = SORTING("G/L Account No.", "Posting Date") ORDER(Ascending) WHERE("G/L Account No." = FILTER('31-01-001100' .. '31-01-045000' | '11-04-335000'), Reversed = FILTER(false), "Source Code" = FILTER(<> 'UNAPPSALES'));
            ...
            column(G_L_Entry__G_L_Entry___Global_Dimension_2_Code_; "G/L Entry"."Global Dimension 2 Code")
            {
            }
            column(Docno; Docno)
            {
            ...
            begin
                Client_Name := '';
                GL_Entry.Reset;
                GL_Entry.SetFilter(GL_Entry."G/L Account No.", "G/L Entry"."G/L Account No.");
                GL_Entry.SetFilter(GL_Entry."Document No.", "G/L Entry"."Document No.");
                ...
                if CustLedEntry.Find('-') then begin
                    ...
                    if CustLedEntry."Currency Code" <> '' then
                ...
                Applied_Date := 0D;
                CustLedEntry2.Reset;
                CustLedEntry2.SetRange(CustLedEntry2."Entry No.", EntNo);
                if CustLedEntry2.Find('-') then begin
                    CustLedEntry2.CalcFields(CustLedEntry2."Amount (LCY)");
                    // Docno := CustLedEntry2."Document No.";
                    Gross_Amt_FC := CustLedEntry2."Amount (LCY)";
                    Gross_Amt_INR := CustLedEntry2.Amount;
                    // Memo_Date := CustLedEntry2."Posting Date";
                    Caseid := CustLedEntry2."Global Dimension 2 Code";
                    Applied_Date := CustLedEntry2."Closed at Date";
                end;
                ...
                GL_Entry.SetFilter(GL_Entry."Document No.", "G/L Entry"."Document No.");
                if GL_Entry.Find('-') then
                    Docno := GL_Entry."Applies-to Doc. No."
                else
                    CustLedEntry.Reset;
                CustLedEntry.SetRange(CustLedEntry."Closed by Entry No.", "G/L Entry"."Entry No.");
                if CustLedEntry.Find('-') then
                    Docno := CustLedEntry."Document No."
            end;
