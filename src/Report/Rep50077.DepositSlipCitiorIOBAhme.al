report 50077 "Deposit Slip CitiorIOB-Ahme"
{
    DefaultLayout = RDLC;
    RDLCLayout = './res/DepositSlipCitiorIOBAhme.rdlc';

    dataset
    {
        dataitem("Collection Detail"; "Collection Detail")
        {
            DataItemTableView = WHERE("Deposit Bank" = FILTER('BANK-006' | 'BANK-007' | 'BANK-008' | 'BANK-009'), Location = FILTER('AHMEDABAD'), "On Submit Receipt No." = FILTER(<> ''));
