report 50059 "DepositSlip HDFC=HDFC-Bang"
{
    DefaultLayout = RDLC;
    RDLCLayout = './res/DepositSlipHDFCHDFCBang.50059.rdlc';

    dataset
    {
        dataitem("Collection Detail"; "Collection Detail")
        {
            DataItemTableView = WHERE("Client Bank Name" = FILTER('*HDFC*'), Location = FILTER('BANGALORE'), "On Submit Receipt No." = FILTER(<> ''));
