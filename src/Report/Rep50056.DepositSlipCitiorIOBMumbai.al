report 50056 "Deposit Slip Citi orIOB-Mumbai"
{
    DefaultLayout = RDLC;
    RDLCLayout = './res/DepositSlipCitiorIOBMumbai.rdlc';

    dataset
    {
        dataitem("Collection Detail"; "Collection Detail")
        {
            DataItemTableView = WHERE("Deposit Bank" = FILTER('BANK-006' | 'BANK-007' | 'BANK-008' | 'BANK-009'), Location = FILTER('MUMBAI'), "On Submit Receipt No." = FILTER(<> ''));
