report 50161 "IBL Tax Draft GST/Credit Memo"
{
    ...
    DefaultLayout = RDLC;
    RDLCLayout = './res/IBLTaxDraftGSTCreditMemo.rdlc';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Header")
        {
            ...
            column(PostCodeNew; PostCodeNew)
            {
            }
            ...
            column(BillDesc; BillDesc)
            {
            }
            column(CreditNote; CreditNote)
            {
            }
            column(Dated; Format(Dated))
            {
            }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Line")
                    {
                        ...
                        begin
                            ...
                            */
                            ...
                            Desc := "Other Detail 1" + ' ' + "Other Detail 2";
                            ...
                            SnoNew += 1;
                            if SnoNew = "Sales Invoice Line".Count then begin
                                CheckReport.InitTextVariable;
                                //CheckReport.FormatNoText(NumberText,"Sales Invoice Line"."Amount Including Tax","Sales Invoice Header"."Currency Code");
                                //CheckReport.FormatNoText(NumberText,ROUND(LineTotAmt + IGSTAmt + CGSTAmt + SGSTAmt,1),"Sales Invoice Header"."Currency Code");
                                CheckReport.FormatNoText(NumberText, Round(LineTotAmt + Abs(IGSTAmt) + Abs(CGSTAmt) + Abs(SGSTAmt), 1), "Sales Invoice Header"."Currency Code");//8112
                                Total_Amnt := Round(LineTotAmt + Abs(IGSTAmt) + Abs(CGSTAmt) + Abs(SGSTAmt), 1);
            ...
            trigger OnAfterGetRecord()
            var
                DetGSTLedger: Record "Detailed GST Entry Buffer";
                StateRec: Record State;
            begin
                CreditNote := false;
                Clear(Dated);

                if "Sales Invoice Header"."Document Type" = "Sales Invoice Header"."Document Type"::"Credit Memo" then begin
                    Heading := 'CREDIT NOTE';
                    CreditNote := true;
                    BillDesc := "Sales Invoice Header"."Bill Description";
                    if SihRec.Get("Sales Invoice Header"."Applies-to Doc. No.") then
                        Dated := SihRec."Posting Date";
                end
                else
                    Heading := 'TAX INVOICE';
                ...
                /*
                //TCPL::6904 220917 start
                IF "Order No." = '' THEN
                  OrderNoText := ''
                ELSE
                  OrderNoText := FIELDCAPTION("Order No.");
                //TCPL::6904 220917 end
                */
                ...
                */
                ...
                if ShiptoAddress.FindFirst then begin
                    ...
                    StateDesc := ShiptoAddress."State Name";
                    PostCodeNew := ShiptoAddress."Post Office";
                    CityName := ShiptoAddress."City Town";
                    StateRec.Reset;
                    StateRec.SetRange(Description, ShiptoAddress."State Name");
                    if StateRec.FindFirst then
                        GSTStateCode := StateRec."State Code (GST Reg. No.)";
                    Mobile := ShiptoAddress."Phone No.";
                end else begin
                    CountryName := "Sales Invoice Header"."Sell-to County";
                    //ShipLocation:=;
                    RecipientGST := Cust."GST Reg No.";
                    StateDesc := "Sales Invoice Header".ShipToStateName;
                    PostCodeNew := "Sales Invoice Header"."Sell-to Post Code";
                    CityName := "Sales Invoice Header"."Sell-to City";
                    StateRec.Reset;
                ...
                DetGSTLedger.Reset;
                DetGSTLedger.SetRange("Document No.", "Sales Invoice Header"."No.");
                //DetGSTLedger.SETRANGE("Document Line No.","Sales Invoice Line"."Line No.");
                if DetGSTLedger.FindFirst then
                    repeat
                        case DetGSTLedger."GST Component Code" of
                            'IGST':
                                begin
                                    IGSTAmt += DetGSTLedger."GST Amount";
                                    IGSTPer := DetGSTLedger."GST %";
                                end;
                            'CGST':
                                begin
                                    CGSTAmt += DetGSTLedger."GST Amount";
                                    CGSTPer := DetGSTLedger."GST %";
                                end;
                            'SGST':
                                begin
                                    SGSTAmt += DetGSTLedger."GST Amount";
                                    SGSTPer := DetGSTLedger."GST %";
                                end;
                        end;
                    until
                    DetGSTLedger.Next = 0;
                ...
                if StateRec.Get(Cust."State Code") then;

                REc_State.Reset;
                REc_State.SetRange(Description, ShiptoAddress."State Name");
                if REc_State.FindFirst then
                    StateCodeVar := ShiptoAddress."State Name" + '-' + REc_State."State Code (GST Reg. No.)"
                else
    ...
    var
        ...
        KINDLY_PAY_IN_FAVOUR_OF__LAKSHMI_KUMARAN___SRIDHARAN__Lbl: Label 'KINDLY PAY IN FAVOUR OF ''INSTITUTE OF BUSINESS LAWS'' AND QUOTE THIS INVOICE NO. WHILE MAKING PAYMENT';
        place1: Decimal;
